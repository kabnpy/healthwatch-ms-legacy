...
def test_read_risk_note_pdf_auth(
    ...
    from app.models import Client, Insurer, Policy, Product, RiskNote, TransactionType
    ...
    rn = RiskNote(
        ...
        cover_snapshot={
            "VEHICLE": {"Reg No": "KDH 519E", "Make": "Toyota Hilux", "Year": 2020},
    ...
    response = client.get(
        f"{settings.API_V1_STR}/risk-notes/{rn.id}/pdf", headers=superuser_token_headers
    )
    assert response.status_code == 200
