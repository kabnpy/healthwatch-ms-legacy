...
def delete_insurer(session: Session, *, id: uuid.UUID) -> None:
    db_obj = session.get(Insurer, id)
    if db_obj:
        db_obj.deleted_at = datetime.now()
        session.add(db_obj)
        session.commit()

def update_insurer(*, session: Session, db_insurer: Insurer, insurer_in: InsurerUpdate) -> Insurer:
...
def delete_product(session: Session, *, id: uuid.UUID) -> None:
    db_obj = session.get(Product, id)
    if db_obj:
        db_obj.deleted_at = datetime.now()
        session.add(db_obj)
        session.commit()
...
def create_correspondence(*, session: Session, correspondence_in: CorrespondenceCreate) -> Correspondence:
...
def get_policies_by_client_id(session: Session, *, client_id: uuid.UUID) -> Sequence[Policy]:
    statement = (
        select(Policy)
...
def update_risk_note(*, session: Session, db_risk_note: RiskNote, risk_note_in: RiskNoteUpdate) -> RiskNote:
...
def create_claim_event(*, session: Session, claim_event_in: ClaimEventCreate) -> ClaimEvent:
...
def create_invoice_bulk(*, session: Session, client_id: uuid.UUID, risk_note_ids: list[uuid.UUID], date_issued: datetime, notes: str | None = None) -> Invoice:
    risk_notes = session.exec(select(RiskNote).where(RiskNote.id.in_(risk_note_ids))).all()
