...
class RiskNote(RiskNoteBase, table=True):
    ...
    policy_snapshot: dict[str, Any] = Field(default_factory=dict, sa_column=Column(JSON))
...
class InvoiceBase(AuditMixin, SQLModel):
    ...
    total_amount: Decimal = Field(default=Decimal("0.0"), sa_type=Numeric(precision=15, scale=2))
    balance_due: Decimal = Field(default=Decimal("0.0"), sa_type=Numeric(precision=15, scale=2))
    notes: str | None = None
...
class ReceiptBase(AuditMixin, SQLModel):
    ...
    date_received: date
    amount: Decimal = Field(sa_type=Numeric(precision=15, scale=2))
    unallocated_amount: Decimal = Field(default=Decimal("0.0"), sa_type=Numeric(precision=15, scale=2))
    mode: str
    reference: str
    notes: str | None = None
...
class ClaimBase(AuditMixin, SQLModel):
    ...
    reserve_amount: Decimal = Field(default=Decimal("0.0"), sa_type=Numeric(precision=15, scale=2))
