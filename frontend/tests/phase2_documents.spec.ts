...
test.describe("Phase 2: Professional Document Redesign", () => {
  ...
  test("Risk Note template has professional elements", async ({ page }) => {
    ...
    await page.waitForSelector("table")
    const policyLink = page.locator('table tbody tr a[href*="/policies/"]').first()
    ...
    if (await policyLink.isVisible()) {
      ...
      await expect(page.getByRole("heading", { name: /Risk Note/i })).toBeVisible()
      
      // Check for letterhead/branding
      await expect(page.getByText("HealthWatch", { exact: false }).first()).toBeVisible()
  ...
  test("Document viewer opens and displays content", async ({ page }) => {
    ...
    if (await policyLink.isVisible()) {
      ...
      await expect(modalTitle.filter({ hasText: /Risk Note|Debit Note/i }).first()).toBeVisible()
    }
  })
})
