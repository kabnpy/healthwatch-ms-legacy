...
test.describe("Phase 3: Component & Copy Audit - Dialog Column Verification", () => {
  ...
  test("Add Client dialog uses single column layout where appropriate", async ({ page }) => {
    ...
    for (let i = 0; i < gridCount; i++) {
      ...
      if (classList?.includes("grid-cols-2") || classList?.includes("md:grid-cols-2")) {
  ...
  test("Add Policy dialog should be audited for columns", async ({ page }) => {
    ...
    const addPolicyButton = page.getByRole("button", { name: /New Policy|Add Policy/i }).first()
