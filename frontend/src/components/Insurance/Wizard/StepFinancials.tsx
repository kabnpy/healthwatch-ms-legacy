...
export function StepFinancials({ defaultValues, onNext, onBack }: StepFinancialsProps) {
  const form = useForm({
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onNext)} className="space-y-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-semibold uppercase text-muted-foreground">Extensions</h3>
              <div className="grid grid-cols-1 gap-4 border rounded-md p-4 bg-muted/10">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex items-center justify-between space-y-0 rounded-lg p-2 border">
                      <FormLabel className="text-base cursor-pointer">Political Violence & Terrorism (0.25%)</FormLabel>
                      <FormControl>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex items-center justify-between space-y-0 rounded-lg p-2 border">
                      <FormLabel className="text-base cursor-pointer">Excess Protector (0.25%)</FormLabel>
                      <FormControl>
                        <Checkbox checked={field.value} onCheckedChange={field.onChange} />
                      </FormControl>
                    </FormItem>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex items-center justify-between space-y-0 rounded-lg p-2 border">
                      <FormLabel className="text-base cursor-pointer">Passenger Liability (Flat KES 500)</FormLabel>
                      <FormControl>
                        <Checkbox checked={field.value} onCheckedChange={field.onChange} />
                      </FormControl>
                    </FormItem>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-0 bg-muted/30 border rounded-lg p-6 space-y-4">
              <h3 className="font-bold text-center border-b pb-2">PREMIUM BREAKDOWN</h3>
              
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span>Basic Premium:</span>
                  <span className="font-mono">{calculation.breakdown.basic.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                </div>
                {calculation.breakdown.extensions.map((ext) => (
                  <div key={ext.name} className="flex justify-between text-blue-600">
                    <span>+ {ext.name}:</span>
                ...
                <div className="border-t pt-2 mt-2">
                  <div className="flex justify-between text-muted-foreground">
                    ...
                    <span>{calculation.breakdown.levies.trainingLevy.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                  </div>
                  <div className="flex justify-between text-muted-foreground">
                    <span>PHCF Levy:</span>
                    <span>{calculation.breakdown.levies.phcf.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                  </div>
                  <div className="flex justify-between text-muted-foreground">
                    <span>Stamp Duty:</span>
                    <span>{calculation.breakdown.levies.stampDuty.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="border-t-2 border-black pt-4 mt-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-bold text-green-700 font-mono">
                    KES {calculation.breakdown.total.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="flex justify-between pt-4 border-t">
          <Button type="button" variant="outline" onClick={onBack}>Back: Asset Details</Button>
