...
export function StepFinancials({
  ...
  const form = useForm({
    ...
    defaultValues: {
      financials: {
        ...
        startDate: defaultValues.financials?.startDate || new Date().toISOString().split("T")[0],
      ...
      extensions: {
        ...
        passengerLiability: defaultValues.extensions?.passengerLiability || false,
  ...
  useEffect(() => {
    if (isMotorPrivate && calculation.breakdown.basic > 0) {
      ...
      const effectiveRate = (calculation.breakdown.basic / (financials.sumInsured || 1)) * 100
      if (effectiveRate !== financials.rate) {
          form.setValue("financials.rate", Number(effectiveRate.toFixed(3)))
      }
    }
  }, [isMotorPrivate, calculation.breakdown.basic, financials.sumInsured])
  ...
  }, [isMotorPrivate, financials.sumInsured])
    ...
    <Form {...(form as any)}>
      <form onSubmit={form.handleSubmit(onNext)} className="space-y-6">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input type="number" {...field} value={field.value || 12} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
                    ...
                    render={({ field }) => (
                      <FormItem className="flex items-center justify-between space-y-0 rounded-lg p-2 border bg-white">
                        <FormLabel className="text-sm cursor-pointer w-full">
                          ...
                          {isHighEnd && <span className="ml-2 text-[10px] font-bold text-emerald-600 block">(INCLUDED)</span>}
                        </FormLabel>
                        <FormControl>
                          <Checkbox
                            checked={!!field.value}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                    <div className="flex justify-between items-center text-sm">
                        <span className="text-slate-400">{isPA ? "Base Premium" : "Basic Premium"}</span>
                    ...
                    <div className="space-y-1">
                        <span className="text-[10px] font-bold uppercase text-slate-500 tracking-tight">Benefits</span>
                        {isMotor && calculation.breakdown.extensions.length > 0 ? (
                            calculation.breakdown.extensions.map((ext) => (
                                ...
                                <span className={ext.included ? "font-bold text-emerald-400" : "font-mono"}>
                                    {ext.included ? "INCLUDED" : ext.amount.toLocaleString(undefined, {
                                    minimumFractionDigits: 2,
                                    })}
                                </span>
                                </div>
                            ...
                            <p className="text-[10px] text-slate-600 italic">No additional benefits selected.</p>
                        )}
                    </div>
                </div>
        ...
        <div className="flex justify-between pt-6 border-t mt-4">
          ...
          <Button type="submit" size="lg" className="px-8">Next: Review & Issue</Button>
        </div>
      </form>
    </Form>
  )
