...
function RiskNoteLoader({ id }: { id: string }) {
  const { data: riskNote } = useSuspenseQuery(getRiskNoteQueryOptions(id)) as {
    data: RiskNotePublic;
  };
...
function InvoiceLoader({ id }: { id: string }) {
  const { data: invoice } = useSuspenseQuery(getInvoiceQueryOptions(id)) as {
    data: InvoicePublic;
  };

  const { data: client } = useSuspenseQuery(
    getClientQueryOptions(invoice.client_id),
  ) as { data: ClientPublic };
...
type DocumentType = "risknote" | "invoice";
